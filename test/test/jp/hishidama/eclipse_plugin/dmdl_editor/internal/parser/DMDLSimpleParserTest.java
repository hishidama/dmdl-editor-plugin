...
public class DMDLSimpleParserTest extends DMDLSimpleParserTestCase {
	...
	public void parse1() {
		...
		DMDLToken expected = model(word("test"), word("="),
				block(word("{"), prop(word("aaa"), word(":"), word("TEXT"), word(";")), word("}")), word(";"));
	...
	public void parse2() {
		...
		DMDLToken expected = model(
				...
				word("="),
				block(word("{"),
						prop(comm("// comment1\n"), desc("desc1"), word("prop1"), word(":"), word("TEXT"), word(";")),
						prop(comm("-- comment2\n"), desc("desc2"), word("prop2"), word(":"), word("LONG"), word(";")),
						word("}")), word(";"));
		assertEqualsToken(expected, token);
	...
	public void parse_annotation() {
		...
		DMDLToken expected = model(
				...
				word("="),
				block(word("{"),
						prop(ann("@line"), word("prop1"), word(":"), word("TEXT"), word(";")),
						prop(ann("@line2"), args(argq("name", "abc"), arg("value", "TRUE")), word("prop2"), word(":"),
								word("LONG"), word(";")),
						prop(ann("@line3"), args(arg("value", arr("123", "456", "\"abc\""))), word("prop3"), word(":"),
								word("INT"), word(";")),
						//
						word("}")), //
				word(";"));
	...
	public void parse_join1() {
		...
		DMDLToken expected = model(word("joined"), word("item_order"), word("="), word("item"), word("%"),
				word("code"), word(","), word("id"), word("+"), word("order"), word("%"), word("item_code"), word(","),
				word("item_id"), word(";"));
		// System.out.println("expected=" + expected);
		// System.out.println("actual  =" + token);
		assertEqualsToken(expected, token);
	...
	public void parse_join2() {
		...
		DMDLToken expected = model(
				...
				word("item"),
				word("->"),
				block(word("{"), prop(word("code"), word("->"), word("code"), word(";")),
						prop(word("price"), word("->"), word("price"), word(";")), word("}")), //
				word("%"),
				word("code"),
				word("+"),
				word("order"),
				word("->"), //
				block(word("{"), prop(word("item_code"), word("->"), word("code"), word(";")),
						prop(word("amount"), word("->"), word("total"), word(";")), word("}")), //
				word("%"), word("code"), word(";"));
		// System.out.println("expected=" + expected);
		// System.out.println("actual  =" + token);
		assertEqualsToken(expected, token);
	...
	public void parse_summarize2() {
		...
		DMDLToken expected = model(
				...
				word("order"),
				word("=>"),
				block(word("{"), prop(word("any"), word("item_code"), word("->"), word("code"), word(";")),
						prop(word("sum"), word("price"), word("->"), word("total"), word(";")),
						prop(word("count"), word("item_code"), word("->"), word("count"), word(";")), word("}")),
				word("%"), word("code"), word(";"));
		// System.out.println("expected=" + expected);
		// System.out.println("actual  =" + token);
		assertEqualsToken(expected, token);
	...
	public void parse_projective1() {
		...
		DMDLToken expected0 = model(word("projective"), word("proj_model"), word("="),
				block(word("{"), prop(word("value"), word(":"), word("INT"), word(";")), word("}")), word(";"));
		DMDLToken expected1 = model(word("conc_model"), word("="), word("proj_model"), word("+"),
				block(word("{"), prop(word("other"), word(":"), word("INT"), word(";")), word("}")), word(";"));
	...
	public void parse_projective2() {
		...
		DMDLToken expected0 = model(word("projective"), word("super_proj"), word("="),
				block(word("{"), prop(word("a"), word(":"), word("INT"), word(";")), word("}")), word(";"));
		DMDLToken expected1 = model(word("projective"), word("sub_proj"), word("="), word("super_proj"), word("+"),
				block(word("{"), prop(word("b"), word(":"), word("INT"), word(";")), word("}")), word(";"));
		DMDLToken expected2 = model(word("record"), word("="), word("sub_proj"), word(";"));
		assertEqualsToken(expected0, list.get(0));
		assertEqualsToken(expected1, list.get(1));
	...
	public void parse_comment() {
		...
		DMDLToken expected0 = model(
				word("item"),
				word("="),
				block(word("{"), prop(word("code"), word(":"), word("LONG"), word(";")),
						prop(comm("-- XYZコード体系で表現される商品コード\n"), word("id"), word(":"), word("TEXT"), word(";")),
						prop(comm("//  name : TEXT;\n")), word("}")), word(";"));
	...
	public void parse_description() {
		...
		DMDLToken expected = model(
				...
				word("="),
				block(word("{"), //
						prop(desc("desc1"), word("prop1"), word(":"), word("TEXT"), word(";")),
						prop(desc("desc\\\"2"), word("prop2"), word(":"), word("LONG"), word(";")), //
						word("}")), word(";"));
		assertEqualsToken(expected, token);
